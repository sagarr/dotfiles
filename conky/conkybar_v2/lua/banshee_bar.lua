...
function conky_main_bars()
	local bars_settings={
		{	--VOLUME Bar (up)
			name="exec",
			arg="~/.conky/conkybar_v2/scripts/conkyBanshee.py --datatype=PP",
			max=100,
			angle=90,
			...
			handle_show=true,
			handle_size=10,
			handle_color={0x1793d0,1},
			skew_y=-20,
			draw_me="${if_running banshee}1${endif}"
		},	
		{	--VOLUME Bar (down)
			name="exec",
			arg="~/.conky/conkybar_v2/scripts/conkyBanshee.py --datatype=PP",
		...
		if t.angle_bar==0 then
			...
			if (t.handle_show) then
				...
				if (y1 >= -t.handle_size) then
					...
					cairo_rectangle(cr,0,y1+t.handle_size,t.width,-t.handle_size)	
				end
				cairo_fill(cr)
				cairo_pattern_destroy(pat)
			end
	...
	local function setup_bar_graph()
		...
		if (t.txt_show and t.angle_bar==0) then
			...
			if (t.blocks == 1) then		--Determine the y position of the txt
				...
				--Inefficient, feel free to replace with a formula
			...
			cairo_save(cr)	
			cairo_select_font_face(cr, t.txt_font, CAIRO_FONT_SLANT_NORMAL, 0)
			...
			-- THIS DOES NOT WORK, FEEL FREE TO FINISH IT (just started)
				--[[	--My intended parameters
					markers_show=false,
					markers_width=5,
					markers_height=20,
					markers_bg={0xffffff,0.25},
					markers_color=true,
					markers_fg={0x77B753,1},
					markers_nb=10,
					markers_offset=5,
					markers_flip=false,
				]]
			--[[
			--block for markers (these should only be redrawn if t.markers_color==true)
			if (t.markers_show) then
				cairo_save(cr)
       				local i = 1
				local pat3 = cairo_pattern_create_rgba(rgb_to_r_g_b(t.markers_bg))
				local pat4 = cairo_pattern_create_rgba(rgb_to_r_g_b(t.markers_fg))
				local pos =0
				for i=1, (t.markers_nb+1), 1 do
					pos=(t.markers_nb / t.max)*i
					if (t.markers_color and pos > (y1+t.markers_width)) then
						cairo_set_source(cr,pat4)
					else
						cairo_set_source(cr,pat3)
					end
					cairo_move_to(cr,t.markers_offset+t.width,y1)
					cairo_line_to(cr,t.width,y1)
					cairo_stroke(cr)
        			end
